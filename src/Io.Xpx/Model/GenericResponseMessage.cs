/* 
 * Proximax P2P Storage REST API
 *
 * Proximax P2P Storage REST API
 *
 * OpenAPI spec version: v0.0.1
 * Contact: alvin.reyes@botmill.io
 * Generated by: https://github.com/swagger-api/swagger-codegen.git
 */

using System;
using System.Linq;
using System.IO;
using System.Text;
using System.Text.RegularExpressions;
using System.Collections;
using System.Collections.Generic;
using System.Collections.ObjectModel;
using System.Runtime.Serialization;
using Newtonsoft.Json;
using Newtonsoft.Json.Converters;
using System.ComponentModel.DataAnnotations;
using SwaggerDateConverter = Io.Xpx.Client.SwaggerDateConverter;

namespace Io.Xpx.Model
{
    /// <summary>
    /// GenericResponseMessage
    /// </summary>
    [DataContract]
    public partial class GenericResponseMessage :  IEquatable<GenericResponseMessage>, IValidatableObject
    {
        /// <summary>
        /// Gets or Sets Status
        /// </summary>
        [JsonConverter(typeof(StringEnumConverter))]
        public enum StatusEnum
        {
            
            /// <summary>
            /// Enum _100 for "100"
            /// </summary>
            [EnumMember(Value = "100")]
            _100,
            
            /// <summary>
            /// Enum _101 for "101"
            /// </summary>
            [EnumMember(Value = "101")]
            _101,
            
            /// <summary>
            /// Enum _102 for "102"
            /// </summary>
            [EnumMember(Value = "102")]
            _102,
            
            /// <summary>
            /// Enum _103 for "103"
            /// </summary>
            [EnumMember(Value = "103")]
            _103,
            
            /// <summary>
            /// Enum _200 for "200"
            /// </summary>
            [EnumMember(Value = "200")]
            _200,
            
            /// <summary>
            /// Enum _201 for "201"
            /// </summary>
            [EnumMember(Value = "201")]
            _201,
            
            /// <summary>
            /// Enum _202 for "202"
            /// </summary>
            [EnumMember(Value = "202")]
            _202,
            
            /// <summary>
            /// Enum _203 for "203"
            /// </summary>
            [EnumMember(Value = "203")]
            _203,
            
            /// <summary>
            /// Enum _204 for "204"
            /// </summary>
            [EnumMember(Value = "204")]
            _204,
            
            /// <summary>
            /// Enum _205 for "205"
            /// </summary>
            [EnumMember(Value = "205")]
            _205,
            
            /// <summary>
            /// Enum _206 for "206"
            /// </summary>
            [EnumMember(Value = "206")]
            _206,
            
            /// <summary>
            /// Enum _207 for "207"
            /// </summary>
            [EnumMember(Value = "207")]
            _207,
            
            /// <summary>
            /// Enum _208 for "208"
            /// </summary>
            [EnumMember(Value = "208")]
            _208,
            
            /// <summary>
            /// Enum _226 for "226"
            /// </summary>
            [EnumMember(Value = "226")]
            _226,
            
            /// <summary>
            /// Enum _300 for "300"
            /// </summary>
            [EnumMember(Value = "300")]
            _300,
            
            /// <summary>
            /// Enum _301 for "301"
            /// </summary>
            [EnumMember(Value = "301")]
            _301,
            
            /// <summary>
            /// Enum _302 for "302"
            /// </summary>
            [EnumMember(Value = "302")]
            _302,
            
            /// <summary>
            /// Enum _303 for "303"
            /// </summary>
            [EnumMember(Value = "303")]
            _303,
            
            /// <summary>
            /// Enum _304 for "304"
            /// </summary>
            [EnumMember(Value = "304")]
            _304,
            
            /// <summary>
            /// Enum _305 for "305"
            /// </summary>
            [EnumMember(Value = "305")]
            _305,
            
            /// <summary>
            /// Enum _307 for "307"
            /// </summary>
            [EnumMember(Value = "307")]
            _307,
            
            /// <summary>
            /// Enum _308 for "308"
            /// </summary>
            [EnumMember(Value = "308")]
            _308,
            
            /// <summary>
            /// Enum _400 for "400"
            /// </summary>
            [EnumMember(Value = "400")]
            _400,
            
            /// <summary>
            /// Enum _401 for "401"
            /// </summary>
            [EnumMember(Value = "401")]
            _401,
            
            /// <summary>
            /// Enum _402 for "402"
            /// </summary>
            [EnumMember(Value = "402")]
            _402,
            
            /// <summary>
            /// Enum _403 for "403"
            /// </summary>
            [EnumMember(Value = "403")]
            _403,
            
            /// <summary>
            /// Enum _404 for "404"
            /// </summary>
            [EnumMember(Value = "404")]
            _404,
            
            /// <summary>
            /// Enum _405 for "405"
            /// </summary>
            [EnumMember(Value = "405")]
            _405,
            
            /// <summary>
            /// Enum _406 for "406"
            /// </summary>
            [EnumMember(Value = "406")]
            _406,
            
            /// <summary>
            /// Enum _407 for "407"
            /// </summary>
            [EnumMember(Value = "407")]
            _407,
            
            /// <summary>
            /// Enum _408 for "408"
            /// </summary>
            [EnumMember(Value = "408")]
            _408,
            
            /// <summary>
            /// Enum _409 for "409"
            /// </summary>
            [EnumMember(Value = "409")]
            _409,
            
            /// <summary>
            /// Enum _410 for "410"
            /// </summary>
            [EnumMember(Value = "410")]
            _410,
            
            /// <summary>
            /// Enum _411 for "411"
            /// </summary>
            [EnumMember(Value = "411")]
            _411,
            
            /// <summary>
            /// Enum _412 for "412"
            /// </summary>
            [EnumMember(Value = "412")]
            _412,
            
            /// <summary>
            /// Enum _413 for "413"
            /// </summary>
            [EnumMember(Value = "413")]
            _413,
            
            /// <summary>
            /// Enum _414 for "414"
            /// </summary>
            [EnumMember(Value = "414")]
            _414,
            
            /// <summary>
            /// Enum _415 for "415"
            /// </summary>
            [EnumMember(Value = "415")]
            _415,
            
            /// <summary>
            /// Enum _416 for "416"
            /// </summary>
            [EnumMember(Value = "416")]
            _416,
            
            /// <summary>
            /// Enum _417 for "417"
            /// </summary>
            [EnumMember(Value = "417")]
            _417,
            
            /// <summary>
            /// Enum _418 for "418"
            /// </summary>
            [EnumMember(Value = "418")]
            _418,
            
            /// <summary>
            /// Enum _419 for "419"
            /// </summary>
            [EnumMember(Value = "419")]
            _419,
            
            /// <summary>
            /// Enum _420 for "420"
            /// </summary>
            [EnumMember(Value = "420")]
            _420,
            
            /// <summary>
            /// Enum _421 for "421"
            /// </summary>
            [EnumMember(Value = "421")]
            _421,
            
            /// <summary>
            /// Enum _422 for "422"
            /// </summary>
            [EnumMember(Value = "422")]
            _422,
            
            /// <summary>
            /// Enum _423 for "423"
            /// </summary>
            [EnumMember(Value = "423")]
            _423,
            
            /// <summary>
            /// Enum _424 for "424"
            /// </summary>
            [EnumMember(Value = "424")]
            _424,
            
            /// <summary>
            /// Enum _426 for "426"
            /// </summary>
            [EnumMember(Value = "426")]
            _426,
            
            /// <summary>
            /// Enum _428 for "428"
            /// </summary>
            [EnumMember(Value = "428")]
            _428,
            
            /// <summary>
            /// Enum _429 for "429"
            /// </summary>
            [EnumMember(Value = "429")]
            _429,
            
            /// <summary>
            /// Enum _431 for "431"
            /// </summary>
            [EnumMember(Value = "431")]
            _431,
            
            /// <summary>
            /// Enum _451 for "451"
            /// </summary>
            [EnumMember(Value = "451")]
            _451,
            
            /// <summary>
            /// Enum _500 for "500"
            /// </summary>
            [EnumMember(Value = "500")]
            _500,
            
            /// <summary>
            /// Enum _501 for "501"
            /// </summary>
            [EnumMember(Value = "501")]
            _501,
            
            /// <summary>
            /// Enum _502 for "502"
            /// </summary>
            [EnumMember(Value = "502")]
            _502,
            
            /// <summary>
            /// Enum _503 for "503"
            /// </summary>
            [EnumMember(Value = "503")]
            _503,
            
            /// <summary>
            /// Enum _504 for "504"
            /// </summary>
            [EnumMember(Value = "504")]
            _504,
            
            /// <summary>
            /// Enum _505 for "505"
            /// </summary>
            [EnumMember(Value = "505")]
            _505,
            
            /// <summary>
            /// Enum _506 for "506"
            /// </summary>
            [EnumMember(Value = "506")]
            _506,
            
            /// <summary>
            /// Enum _507 for "507"
            /// </summary>
            [EnumMember(Value = "507")]
            _507,
            
            /// <summary>
            /// Enum _508 for "508"
            /// </summary>
            [EnumMember(Value = "508")]
            _508,
            
            /// <summary>
            /// Enum _509 for "509"
            /// </summary>
            [EnumMember(Value = "509")]
            _509,
            
            /// <summary>
            /// Enum _510 for "510"
            /// </summary>
            [EnumMember(Value = "510")]
            _510,
            
            /// <summary>
            /// Enum _511 for "511"
            /// </summary>
            [EnumMember(Value = "511")]
            _511
        }

        /// <summary>
        /// Gets or Sets Status
        /// </summary>
        [DataMember(Name="status", EmitDefaultValue=false)]
        public StatusEnum? Status { get; set; }
        /// <summary>
        /// Initializes a new instance of the <see cref="GenericResponseMessage" /> class.
        /// </summary>
        /// <param name="Cause">Cause.</param>
        /// <param name="LocalizedMessage">LocalizedMessage.</param>
        /// <param name="Message">Message.</param>
        /// <param name="StackTrace">StackTrace.</param>
        /// <param name="Status">Status.</param>
        /// <param name="Suppressed">Suppressed.</param>
        public GenericResponseMessage(Throwable Cause = default(Throwable), string LocalizedMessage = default(string), string Message = default(string), List<StackTraceElement> StackTrace = default(List<StackTraceElement>), StatusEnum? Status = default(StatusEnum?), List<Throwable> Suppressed = default(List<Throwable>))
        {
            this.Cause = Cause;
            this.LocalizedMessage = LocalizedMessage;
            this.Message = Message;
            this.StackTrace = StackTrace;
            this.Status = Status;
            this.Suppressed = Suppressed;
        }
        
        /// <summary>
        /// Gets or Sets Cause
        /// </summary>
        [DataMember(Name="cause", EmitDefaultValue=false)]
        public Throwable Cause { get; set; }

        /// <summary>
        /// Gets or Sets LocalizedMessage
        /// </summary>
        [DataMember(Name="localizedMessage", EmitDefaultValue=false)]
        public string LocalizedMessage { get; set; }

        /// <summary>
        /// Gets or Sets Message
        /// </summary>
        [DataMember(Name="message", EmitDefaultValue=false)]
        public string Message { get; set; }

        /// <summary>
        /// Gets or Sets StackTrace
        /// </summary>
        [DataMember(Name="stackTrace", EmitDefaultValue=false)]
        public List<StackTraceElement> StackTrace { get; set; }


        /// <summary>
        /// Gets or Sets Suppressed
        /// </summary>
        [DataMember(Name="suppressed", EmitDefaultValue=false)]
        public List<Throwable> Suppressed { get; set; }

        /// <summary>
        /// Returns the string presentation of the object
        /// </summary>
        /// <returns>String presentation of the object</returns>
        public override string ToString()
        {
            var sb = new StringBuilder();
            sb.Append("class GenericResponseMessage {\n");
            sb.Append("  Cause: ").Append(Cause).Append("\n");
            sb.Append("  LocalizedMessage: ").Append(LocalizedMessage).Append("\n");
            sb.Append("  Message: ").Append(Message).Append("\n");
            sb.Append("  StackTrace: ").Append(StackTrace).Append("\n");
            sb.Append("  Status: ").Append(Status).Append("\n");
            sb.Append("  Suppressed: ").Append(Suppressed).Append("\n");
            sb.Append("}\n");
            return sb.ToString();
        }
  
        /// <summary>
        /// Returns the JSON string presentation of the object
        /// </summary>
        /// <returns>JSON string presentation of the object</returns>
        public string ToJson()
        {
            return JsonConvert.SerializeObject(this, Formatting.Indented);
        }

        /// <summary>
        /// Returns true if objects are equal
        /// </summary>
        /// <param name="obj">Object to be compared</param>
        /// <returns>Boolean</returns>
        public override bool Equals(object obj)
        {
            // credit: http://stackoverflow.com/a/10454552/677735
            return this.Equals(obj as GenericResponseMessage);
        }

        /// <summary>
        /// Returns true if GenericResponseMessage instances are equal
        /// </summary>
        /// <param name="other">Instance of GenericResponseMessage to be compared</param>
        /// <returns>Boolean</returns>
        public bool Equals(GenericResponseMessage other)
        {
            // credit: http://stackoverflow.com/a/10454552/677735
            if (other == null)
                return false;

            return 
                (
                    this.Cause == other.Cause ||
                    this.Cause != null &&
                    this.Cause.Equals(other.Cause)
                ) && 
                (
                    this.LocalizedMessage == other.LocalizedMessage ||
                    this.LocalizedMessage != null &&
                    this.LocalizedMessage.Equals(other.LocalizedMessage)
                ) && 
                (
                    this.Message == other.Message ||
                    this.Message != null &&
                    this.Message.Equals(other.Message)
                ) && 
                (
                    this.StackTrace == other.StackTrace ||
                    this.StackTrace != null &&
                    this.StackTrace.SequenceEqual(other.StackTrace)
                ) && 
                (
                    this.Status == other.Status ||
                    this.Status != null &&
                    this.Status.Equals(other.Status)
                ) && 
                (
                    this.Suppressed == other.Suppressed ||
                    this.Suppressed != null &&
                    this.Suppressed.SequenceEqual(other.Suppressed)
                );
        }

        /// <summary>
        /// Gets the hash code
        /// </summary>
        /// <returns>Hash code</returns>
        public override int GetHashCode()
        {
            // credit: http://stackoverflow.com/a/263416/677735
            unchecked // Overflow is fine, just wrap
            {
                int hash = 41;
                // Suitable nullity checks etc, of course :)
                if (this.Cause != null)
                    hash = hash * 59 + this.Cause.GetHashCode();
                if (this.LocalizedMessage != null)
                    hash = hash * 59 + this.LocalizedMessage.GetHashCode();
                if (this.Message != null)
                    hash = hash * 59 + this.Message.GetHashCode();
                if (this.StackTrace != null)
                    hash = hash * 59 + this.StackTrace.GetHashCode();
                if (this.Status != null)
                    hash = hash * 59 + this.Status.GetHashCode();
                if (this.Suppressed != null)
                    hash = hash * 59 + this.Suppressed.GetHashCode();
                return hash;
            }
        }

        /// <summary>
        /// To validate all properties of the instance
        /// </summary>
        /// <param name="validationContext">Validation context</param>
        /// <returns>Validation Result</returns>
        IEnumerable<System.ComponentModel.DataAnnotations.ValidationResult> IValidatableObject.Validate(ValidationContext validationContext)
        {
            yield break;
        }
    }

}
